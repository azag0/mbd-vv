...
vdw_params_s66 = pd.concat(
    dict(rp.evaluate_mbd_specs(
        [
            ...
            {'vv': True, 'C_vv': 0.0093, 'beta': 0.83, '_label': 'VV'},
            {'vv': 'nm', 'C_vv': 0.0093, 'beta': 0.83, '_label': 'nmVV'},
            {'vv': 'lg', 'C_vv': 0.0093, 'beta': 0.83, '_label': 'lgVV'},
            {'vv': 'lg2', 'C_vv': 0.0093, 'beta': 0.83, '_label': 'lg2VV'},
        ],
        aims_data_s66,
        alpha_vvs_s66,
        get_vdw_params=True,
...
free_atoms_comparison = (
    ...
    .merge(rp.vdw_params, left_on='species', right_index=True)
).sort_values('N')

# ::>


def plot_species(ax, df, what, label=None):
    ax.plot(df['N'], df[what], label=label)


fig, ax = plt.subplots()
plot_species(ax, free_atoms_comparison.loc(0)[0.0093, False], 'C6(TS)', 'TS')
plot_species(ax, free_atoms_comparison.loc(0)[0.0093, False], 'C6(BG)', 'BG')
plot_species(ax, free_atoms_comparison.loc(0)[0.0093, False], 'C6', 'VV')
ax.set_yscale('log')
ax.set_xlabel('$Z$')
ax.set_ylabel('$C_6$')
ax.legend()
...
C6s_from_methods = pd.concat([
    ...
    pd.concat({
        ...
        'VV(normed,spherical)': rp.evaluate_func_on_C6(
            my_pol, C6_set_pts, ref_C6,
            free_atoms_pts.loc[True], coords_C6.species, rescale=True
        ),
...
free_atoms_vv = rp.free_atoms_vv(free_atoms_pts)
...
energies_s66_vvnorm = rp.specs_to_binding_enes([
    {},
    # {'scs': True, 'beta': 0.83},
    # {'vv': True, 'vv_norm': True, 'beta': 0.8},  # optimal beta
    # almost equivalent
    # {'vv': True, 'C_vv': 0.0093, 'vv_norm': 'sph', 'beta': 0.8},
    #
    # {'vv': 'nm', 'C_vv': 0.0093, 'vv_norm': 'sph', 'beta': 0.76},
    # {'rpa': True, 'C_vv': 0.0093, 'ord2': True, 'vv': 'nm', 'vv_norm': 'sph', 'beta': 0.76},
    {'vv': True, 'C_vv': 0.0093, 'beta': 0.88, 'Rvdw17_base': True},  # optimal beta
    {'vv': True, 'vv_norm': 'nonsph', 'C_vv': 0.0093, 'beta': 0.88, 'Rvdw17_base': True},  # optimal beta
    {'vv': True, 'vv_norm': 'sph', 'C_vv': 0.0093, 'beta': 0.88, 'Rvdw17_base': True},  # optimal beta
    {'vv': 'lg2', 'vv_norm': 'nonsph', 'C_vv': 0.0093, 'beta': 0.77, 'Rvdw17_base': True},  # optimal beta
    {'vv': 'lg2', 'vv_norm': 'sph', 'C_vv': 0.0093, 'beta': 0.77, 'Rvdw17_base': True},  # optimal beta
...
# - eigenvalue scaling
# - MBD 2nd order
# - subtracting metallic polarizability
# - how to define vdw radii
#
# ### Polarizability
#
# - VV10 optimization
# - adding kinetic energy inside functional
# - normalization
# - separating metallic polarizability
# - use for vdw radii
# - metallic surface
